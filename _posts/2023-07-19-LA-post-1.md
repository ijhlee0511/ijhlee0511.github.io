...
In this post, we will talk about canonical isomorprhisms with non-canonical looks like inverses related to tensor products in linear algebra. I worte this simple post to review some linear algebra and test [MathJax](https://www.mathjax.org).
...
By tedious computations, one can show $$d_\star$$ is actually the two-sided inverse of $$c_\star$$. Similar to previous example, the construction of $$d_\star$$ looks dependent to the choice of $$\{e_1, \ldots, e_n\}$$, but it does not in fact. Therefore, we can **naturally identify** $$(V^{\otimes n})^\ast$$ with $$(V^\ast)^{\otimes n}$$, though the expression of $$d_\star$$ involves $$\{e_1, \ldots, e_n\}$$. (Can you show that this identification may fail if $$V$$ is not a finite dimensional?)
