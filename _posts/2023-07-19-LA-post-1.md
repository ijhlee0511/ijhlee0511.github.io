...
Let's move on to another example. Let $$V$$ be a vector space over $$k$$, and $$V^{\otimes n} := \underbrace{V \otimes \cdots \otimes V}_{n\text{-times}}$$ and $$V^{\otimes 0} := k$$. If $$I = (i_1, \ldots, i_n)$$ and $$J = (j_1, \ldots, j_n)$$ are two sequences of $$n$$ distinct indices from $$\{1, \ldots, p\}$$ and $$\{1, \ldots, q\}$$, respectively, we define a contraction map
...
In linear algebra, there are lots of objects for which definition seems to depend on the choice of basis, but actually not. Important examples taught in typical undergraduate linear algebra courses are determinant and trace. We define the determinant and trace of linear transformation $$T$$ using their matrix representation (i.e., using bases), but the values do not be affected by the choice of basis. It is important to notice that a non-canonical-looking expression does not imply it is non-canonical.
